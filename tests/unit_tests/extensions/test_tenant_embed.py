...
The ``build_tenant_rls_rule`` function returns an RLS rule payload with the
standard ``dataset`` and ``clause`` fields.  These tests verify that the
payload is structurally correct and that the RLS rule passes validation by
the upstream guest-token schema.
"""
